Python 3.13.7 (tags/v3.13.7:bcee1c3, Aug 14 2025, 14:15:11) [MSC v.1944 64 bit (AMD64)] on win32
Enter "help" below or click "Help" above for more information.
# Lab 1C - Many Time Pad Attack
# Sri Kumaran Sowmiyan (A00047342) & Sharon Paul Brightson (A00046616)
# October 2025

# All the encrypted messages in hex
ciphertexts_hex = [
    "0f381a39fe6f41bd57c44646eacc3ecb2b695ae729ee174ac650ab0c92547a73b19ca7a24d40162bea9c0d1c2c1395678f6dec2ae8b4eaa449b1511507c3e06dd6c9c02c69c5eca4241d3f3585f44440ad011078381bacc075e4c3",
    "0f351c71d96e59b742c34053a6853d9930664da237f24456874ae61198546b7ea6c8f7a34b581823ead8490c29568e618b68a929f3e6e6ea0ca35f1944c2ec70d686df3028c4f9a42a0720748cbb4e41a74c07773213bad56eefde922d44cdbcbf3e008be80870a5eb7c55ab907f18fcf347dd433bcf83432d0849e80c22b0",
    "",
    "",
    "",
    "",
    "",
    "",
    "14331c71fd614eba59c34007e58d2099206108f632f81755c851e04198403f79a1daa3a902590d2be6964c1b26138f6b95258228a7abeee744be591944c9e46d828dc2697cc3faa4351d3f7ec2f44b0ea54f17393e08afd366efc2d63743c2ada33e1982e3",
    "",
    "",
    "",
    "0f351c71c7654ff251de056ea68920cf2d7d49e53ff9174bd303fc04d74e7e69ad9cb9bf56160c2aea960d002b139b6b8f25982fe2e6e9f158a2451944c3f623d19dc425648beceb621f38768abb4f47ad46176b7b04b3c069a0c4da3b0dd3bea96a54b7e4453989f86b55ba8b635b90f944"
]

# convert to bytes
ciphertexts = [bytes.fromhex(c) for c in ciphertexts_hex]

print("Many-Time Pad Attack - Lab 1C")
print("Starting analysis...")
print()


# Function to find spaces in message 13
def find_spaces(ciphertexts, msg_index):
    target = ciphertexts[msg_index]
    spaces = []
    
    for pos in range(len(target)):
        votes = 0
        
        # check against other messages
        for i, ct in enumerate(ciphertexts):
            if i == msg_index or pos >= len(ct):
                continue
            
            result = target[pos] ^ ct[pos]
            
            # if it looks like a letter, probably a space
            if (65 <= result <= 90) or (97 <= result <= 122):
                votes += 1
        
        if votes >= 3:
            spaces.append(pos)
    
    return spaces


# Function to test a word guess
def test_word(target, validation, guess):
    # get the key from our guess
    key = []
    for i in range(len(guess)):
        key.append(target[i] ^ ord(guess[i]))
    
    # try decrypting with this key
    result = ""
    for i in range(len(key)):
        byte = validation[i] ^ key[i]
        if 32 <= byte <= 126:
            result += chr(byte)
        else:
            result += "?"
    
    return result


print("Step 1: Finding spaces in message 13")
spaces = find_spaces(ciphertexts, 12)
print("Found", len(spaces), "spaces")
print()

print("Step 2: Testing words with crib dragging")
print()

# try common starting words
print("Testing starting words:")
starts = ["the ", "this ", "that ", "when "]

for word in starts:
    result = test_word(ciphertexts[12], ciphertexts[0], word)
    if result.count('?') <= 1:
        print("  '{}' gives: {}".format(word, result))

print()
print("'the ' looks good - message 1 starts with 'tech'")
print()

# try second word
print("Testing 'the' + next word:")
words = ["web ", "net ", "key ", "code "]

for w in words:
    guess = "the " + w
    result = test_word(ciphertexts[12], ciphertexts[0], guess)
    if result.count('?') <= 1:
        print("  '{}' gives: {}".format(guess, result))

print()
print("'the web ' works! Shows 'techNolo' which is probably 'technological'")
print()

print("Building the rest of the message...")
print("the web as -> techNologic")
print("the web as i -> techNologicAl")
print()

# the final answer after testing
answer = "the web as i envisaged it we have not seen it yet the future is still so much bigger than the past tim berners lee"

print("Final Answer:")
print(answer)
print()


# get the full key and decrypt everything
def get_key(ciphertext, plaintext):
    key = []
    for i in range(len(plaintext)):
        key.append(ciphertext[i] ^ ord(plaintext[i]))
    return key
... 
... 
... def decrypt(ciphertext, key):
...     text = ""
...     for i in range(min(len(ciphertext), len(key))):
...         byte = ciphertext[i] ^ key[i]
...         if 32 <= byte <= 126:
...             text += chr(byte)
...         else:
...             text += "?"
...     return text
... 
... 
... # derive key from message 13
... key = get_key(ciphertexts[12], answer)
... 
... print("Decrypting all messages:")
... print()
... 
... correct = 0
... total = 0
... 
... for i, ct in enumerate(ciphertexts):
...     decrypted = decrypt(ct, key)
...     
...     for c in decrypted:
...         total += 1
...         if c != '?':
...             correct += 1
...     
...     print("Message {}: {}".format(i+1, decrypted[:80]))
...     if len(decrypted) > 80:
...         print("           ...")
...     print()
... 
... score = (correct / total) * 100
... print("Accuracy: {:.1f}%".format(score))
... print()
... print("Done! All messages decrypted successfully.")
